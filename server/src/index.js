...
async function ensureSchema() {
  ...
  // Backward-compatible migrations (for older deployments that had a simpler `channels` table)
  // IMPORTANT:
  //  - CREATE TABLE IF NOT EXISTS does NOT add new columns on existing tables.
  //  - We keep these ALTERs safe/idempotent so the server can start even with an old DB volume.
  //  - We avoid adding NOT NULL / FK constraints here, because existing rows would break startup.
  ...
  await pool.query(`UPDATE channels SET created_at = $1 WHERE created_at IS NULL;`, [now]);
...
// Per-user settings (client customization, pinned chats, etc.)
await pool.query(`
...
app.get('/me', authMiddleware, (req, res) => {
  res.json({ user: { id: String(req.user?.sub), username: String(req.user?.username ?? req.user?.sub) } });
});

app.get('/me/settings', authMiddleware, async (req, res) => {
...
// PATCH /me/settings with a JSON object to shallow-merge into saved settings.
app.patch('/me/settings', authMiddleware, async (req, res) => {
  if (!pool) return res.status(500).json({ error: 'db not configured' });
  const userId = String(req.user?.sub);
...
try {
  ...
  console.warn('[WARN] failed to upsert user_settings during login', e);
}
...
// Current user's membership + permissions in a server
app.get('/servers/:serverId/me', authMiddleware, async (req, res) => {
  if (!pool) return res.status(500).json({ error: 'db not configured' });
  const serverId = String(req.params.serverId);
  const userId = String(req.user?.sub);
